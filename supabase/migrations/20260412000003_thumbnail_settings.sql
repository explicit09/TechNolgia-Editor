create table shorts_app.thumbnail_settings (
    short_id uuid primary key references shorts_app.shorts(id) on delete cascade,
...
alter table shorts_app.thumbnail_settings enable row level security;

create policy "anon read thumbnail settings"
on shorts_app.thumbnail_settings for select
using (true);
...
on shorts_app.thumbnail_settings for update
using (true)
with check (true);

create or replace function shorts_app.thumbnail_settings_touch_updated_at()
returns trigger as $$
begin
    new.updated_at = now();
    return new;
...
before update on shorts_app.thumbnail_settings
for each row execute function shorts_app.thumbnail_settings_touch_updated_at();
